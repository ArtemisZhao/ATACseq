...
p<-add_argument(p, "input_file_2", default = NULL, help="Second reads file for paired-end reads")
p<-add_argument(p, "prefix", help="Prefix for output bam files; usually <sample_id>")
p<-add_argument(p, "seqnames", default="chr20", help="Reads mapping to specific seqnames")
...
indexForSubread <- gsub("\\.fa$", "", argv$genome)
...
atacReads<-readGAlignmentPairs(sortedBAM,param=ScanBamParam(mapqFilter = 1,
          flag=scanBamFlag(isPaired = TRUE, isProperPair = TRUE),what=c("qnames",
